...
class QuranicExtraction(object):
    def __init__(
        ...
        if self.model_type == "exact":
            if precompiled_patterns != "prebuilt":
                if precompiled_patterns == "build_and_use":
                    self.initialize_from_scratch(
                        create_compiled_patterns=True, save_compiled_patterns=True
                    )
                    self.use_precompiled_patterns = True
                elif precompiled_patterns == "off":
                    self.initialize_from_scratch(
                        create_compiled_patterns=False, save_compiled_patterns=False
                    )
                ...
                with open(
                    "quranic_extractions/pickles/qbigram_compiled.pickle", "rb"
                ) as f:
                ...
                if not os.path.exists(
                    "quranic_extractions/pickles/verses_rules_compiled.pickle"
                ):
                    with zipfile.ZipFile(
                        "quranic_extractions/pickles/verses_rules_compiled.zip", "r"
                    ) as zip_ref:
                        zip_ref.extractall("quranic_extractions/pickles/")
                with open(
                    "quranic_extractions/pickles/verses_rules_compiled.pickle", "rb"
                ) as f:
            ...
            self.quran_df = pd.read_csv(
                "quranic_extractions/data/Quran.txt",
                sep="##|\t",
                names=["sore", "aye", "text"],
                engine="python",
            )
            self.quran_df["text_norm"] = self.quran_df["text"].apply(
    ...
    def remove_extra_chars(self, text, remove_extra_space=True):
        ...
        text = text.translate(
            str.maketrans(replacing_words, len(replacing_words) * " ")
        )
        return text
    ...
    def align_and_get_span(self, input, input_normd, output_bag):
        ...
        output_bag = sorted(
            output_bag,
            key=lambda x: x["input_span"][1] - x["input_span"][0],
            reverse=True,
        )
        for out_ind, output in enumerate(output_bag):
            ...
            for start_ind, inp_rplcd_chNormd_token in enumerate(
                input_rplcd_chNormd_spltd
            ):
                if inp_rplcd_chNormd_token == input_normd_qp_spltd[0]:
                    ...
                    for c_ind, inp_nrmd_qp_tkn in enumerate(input_normd_qp_spltd):
                        if (
                            input_rplcd_chNormd_spltd[start_ind + c_ind]
                            != inp_nrmd_qp_tkn
                        ):
                            flag = False
                    if flag:
                        intervals = input_rplcd_splt_index[
                            start_ind : start_ind + c_ind + 1
                        ]
                        output_bag[out_ind]["input_span"] = [
                            intervals[0][0],
                            intervals[-1][-1],
                        ]
                        output_bag[out_ind]["extracted"] = input[
    ...
    def rule_maker(self, verse, qbigram_text, index):
        ...
        for j in range(0, index):
            ...
            left_regexd, neet_to_handle_va = self.regexitize_verse(
                " ".join(sentencelist[j:index]), without_last_space=without_last_space
            )
            before_bigram = f"(^|{va}|{va} | )" if neet_to_handle_va else ""
            rule = (
                ")(?:" + before_bigram + qbigram_text[2:] + ")"
                if neet_to_handle_va and j == 0
                else rule
            )
        ...
        for j in range(len(sentencelist), index + 2, -1):
            ...
            right_regexd, _ = self.regexitize_verse(
                " ".join(sentencelist[index + 2 : j]),
                va_before=va_end_bigram,
    ...
    def initialize_from_scratch(
        self, create_compiled_patterns=False, save_compiled_patterns=False
    ):
        ...
        "Read quranic data"
        quran_df_index = pd.read_csv(
            "quranic_extractions/data/Quran.txt", names=["id", "text"], sep="\t"
        )["id"]
        self.quran_df = pd.read_csv(
            "quranic_extractions/data/Quran.txt",
            sep="##|\t",
        ...
        if create_compiled_patterns:
            ...
            for qbigram in tqdm(qbigram_found_list):
                for inner_tup in qbigram[1]:
                    ...
                    self.verses_rules_compiled[f"{id}-{index}"] = re.compile(
                        self.rule_maker(verse, qbigram_text, index)
                    )
        if save_compiled_patterns:
            with open(
                "quranic_extractions/pickles/verses_rules_compiled.pickle", "wb"
            ) as f:
    ...
    def extract_verse_exact(self, input_normd, input, use_precompiled_patterns=True):
        ...
        if use_precompiled_patterns:
            for qc in self.qbigram_compiled:
                if len(qc.findall(input_normd)) != 0:
                    qbigram_found_list.append(
                        [qc.pattern, self.qbigram_bag[qc.pattern]]
                    )
        else:
            qbigram_bag_keys = list(self.qbigram_bag.keys())
            for qbigram in qbigram_bag_keys:
        ...
        for qbigram in qbigram_found_list:
            for inner_tup in qbigram[1]:
                ...
                if use_precompiled_patterns:
                    matches = list(
                        self.verses_rules_compiled[f"{id}-{index}"].finditer(
                            input_normd
                        )
                    )
                else:
                ...
                if len(matches) != 0:
                    for match in matches:
                        ...
                        case_ind = 0
                        while (
                            case_ind < len(covered_input_index)
                            and len(covered_input_index) != 0
                        ):
                            ...
                            if (case[0] <= new_range[0] and new_range[1] < case[1]) or (
                            ...
                            ) or (new_range[0] < case[0] and case[1] <= new_range[1]):
                                del covered_input_index[case_ind]
                                del output_bag[case_ind]
                            elif (
                                new_range[0] < case[0]
                                and new_range[1] <= case[1]
                                and case[0] < new_range[1]
                            ):
                                ...
                                if new_len > case_len:
                                    ...
                                    if (
                                        len(
                                            input_normd[
                                                covered_input_index[case_ind][
                                                    0
                                                ] : covered_input_index[case_ind][1]
                                            ]
                                            .strip()
                                            .split(" ")
                                        )
                                    ...
                                    if (
                                        len(
                                            input_normd[new_range[0] : new_range[1]]
                                            .strip()
                                            .split(" ")
                                        )
                                        < 2
                                ...
                                case[0] <= new_range[0]
                                and new_range[0] < case[1]
                                and case[1] < new_range[1]
                            ):
                                new_len = new_range[1] - new_range[0]
                                case_len = case[1] - case[0]
                                if new_len > case_len:
                        ...
                        res = {
                            "input_span": [new_range[0], new_range[1]],
    ...
    def words_similarity(self, w1, w2):
        ...
        res = (
            self.TUPLE_SIMILARITY_FACTOR * sameTupleCount
            - self.CHAR_DIFF_FACTOR * charDiff
        ) / (len(w1) + len(w2))
        ...
        for word in normalizedWords:
            ...
            for inx in self.wordsMap[word]["ayes"]:
                if inx in result:
                    continue
                result[inx] = (
                    self.check_aye_similarity(self.ayes[inx], normalizedWords)
                    - len(self.ayes[inx]["text_norm"].split()) * self.AYE_LEN_FACTOR
                )
        sortedIndexes = sorted(result, key=result.get, reverse=True)[
            : self.SAME_AYE_THRESHOLD
        ]
        if len(sortedIndexes) > 0:
            maxValue = result[sortedIndexes[0]]
            if (
                len(sortedIndexes) > self.SAME_AYE_THRESHOLD
                and result[sortedIndexes[self.SAME_AYE_THRESHOLD]] * self.SAME_AYE_RATIO
                > maxValue
            ):
                return
            for inx in sortedIndexes:
                if maxValue > result[inx] * self.SAME_AYE_RATIO or (
                    len(normalizedWords) > 1
                    and result[inx] < self.MIN_ACCEPT_SIMILARITY
                ):
                    break
                ayat.add(
                    (
                        self.ayes[inx]["text"],
                        str(self.ayes[inx]["sore"]) + "##" + str(self.ayes[inx]["aye"]),
                        round(result[inx], 2),
                    )
                )
    ...
    def run(self, text):
        ...
        if self.model_type == "exact":
            return self.extract_verse_exact(
                input_normd, text, self.use_precompiled_patterns
            )
        elif self.model_type == "apprx":
