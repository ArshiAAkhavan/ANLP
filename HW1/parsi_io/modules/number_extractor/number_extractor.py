...
class NumberExtractor:
    ...
    def __extract_spans(
        ...
        return_value = []
        normed_input_sentence = self.re_norm_add_space.sub(
            r" \1 ", input_sentence
        )  # adding space around digits
        span_convert_dict = self.__find_index_match(
            input_sentence, normed_input_sentence
        )
    ...
    def __get_value_extend(self, sub_phrase: str) -> tuple:
        ...
        before_extend_value = self.__get_value(
            before_extend
        )  # a digit_Value or a simple value
        sub_phrase = self.re_before_extend.sub(
            "", sub_phrase, 1
        )  # remove before extend part

        extends = self.re_extend.findall(sub_phrase)

        if (
            before_extend.strip() == TEXT_SI and extends[0] == TEXT_SAD
        ):  # 300 special case
        ...
        for index, value in enumerate(sub_phrase_values):
            ...
            if temp_value != 0:
                if temp_value % 10 ** math.ceil(math.log(addition, 10)) >= 1:
                    raise ValueError(
                        index
                    )  # Panic: There are more than one valid number in the phrase
    ...
    def run(self, input_sentence):
        ...
        while i < len(spans):
            ...
            try:
                ...
                sub_span2 = (
                    _sub_span2[0] + span[0] + phrase1_end,
                    _sub_span2[1] + span[0] + phrase1_end,
                )
                spans[i] = sub_span1
