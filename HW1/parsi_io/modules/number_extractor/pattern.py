...
PATTERN_DIGIT = join_patterns(
    list(
        ENGLISH_NON_ZERO_DIGITS
        + PERSIAN_NON_ZERO_DIGITS
        + ENGLISH_ZERO_DIGIT
        + PERSIAN_ZERO_DIGIT
    )
)
...
PATTERN_BEFORE_EXTEND = join_patterns(
    [
        rf"{PATTERN_NUMBER_WITH_DIGITS}{WHITE_SPACE}*",
        rf"{PATTERN_SIMPLE_NUMBER}{WHITE_SPACE}*",
    ]
)
PATTERN_EXTEND = join_patterns(PERSIAN_REQUIRED_EXTENDABLE_NUMBERS)
PATTERN_EXTENDABLE_NUMBER = (
    rf"{PATTERN_BEFORE_EXTEND}{PATTERN_EXTEND}(?:{WHITE_SPACE}+{PATTERN_EXTEND})*"
)
...
PATTERN_SINGLE_NUMBER = join_patterns(
    ALL_PARTS.values()
)  # We do not need the keys here
PATTERN_MULTIPLE_NUMBER = rf"{PATTERN_SINGLE_NUMBER}(?:{WHITE_SPACE}+{PERSIAN_V}{WHITE_SPACE}+{PATTERN_SINGLE_NUMBER})*"

ALL_NEGS = join_patterns(
    [rf"{PERSIAN_NEG}{WHITE_SPACE}+", rf"{DIGIT_NEG}{WHITE_SPACE}*"]
)
PATTERN_ALL_NUMBER_EXCEPT_ZERO = f"{ALL_NEGS}?{PATTERN_MULTIPLE_NUMBER}"
PATTERN_ALL_NUMBER = join_patterns(
    [PATTERN_ALL_NUMBER_EXCEPT_ZERO] + list(PERSIAN_SPECIAL_CASES.keys())
)

BEFORE_NUMBER = r"(?:\W|^)"

END_WORD_LIST = ["م", "مین", "بار", "عدد", "برابر"]
