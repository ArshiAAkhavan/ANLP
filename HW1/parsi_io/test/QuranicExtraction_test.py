...
class TestQuranicExtraction(BaseTest):
    ...
    def run_test(self, obj, addr):
        ...
        for i in test_cases:
            ...
            your_initial_answer = sorted(
                your_initial_answer, key=lambda x: x["input_span"][0]
            )
            ...
            d = {
                k: (your_answer[k], correct_answer[k])
                for k in your_answer
                if k in correct_answer and your_answer[k] != correct_answer[k]
            }
            for j in d:
                errors.append(
                    "Input {0}: your answer is {1} correct answer is {2}".format(
                        i["id"], d[j][0], d[j][1]
                    )
                )
